...
CAV_EDGE_PREFIXES = {"40.0.00", "39.0.00", "1185.0.00", "1184.0.00", "-40.0.00", "-39.0.00", "-1185.0.00", "-1184.0.00"}  # adjust to your real ones
...
def is_cav_now(veh_id: str) -> bool:
    try:
        ...
        # ignore internal lanes inside junctions (":...")
        if lane_id.startswith(":"):
            return False

        edge_id = lane_id.rsplit("_", 1)[0]   # "-40.0.00_3" -> "-40.0.00"
...
def get_observed_vehicle_ids(
    ...
    for cav in cavs_now:
        ...
        data = _build_los_frame_for_cav(cav, neigh, positions, dims, headings, perception_range)
